...
describe('file review smoke', () => {
  it('captures, undoes, and reapplies created and deleted files end to end', async () => {
    ...
    expect((await lstat(deletedPath)).mode & 0o777).toBe(process.platform === 'win32' ? 0o666 : 0o600)
    ...
    expect((await lstat(createdPath)).mode & 0o777).toBe(process.platform === 'win32' ? 0o666 : 0o640)
