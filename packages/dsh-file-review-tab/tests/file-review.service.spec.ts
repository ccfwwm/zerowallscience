...
describe('Host file-review change engine', () => {
  it('undoes and reapplies a created UTF-8 file', async () => {
    ...
    expect((await lstat(filename)).mode & 0o777).toBe(process.platform === 'win32' ? 0o666 : 0o640)
  })
      ...
      diffs: [
        {
          ...
          oldText: null,
          newText: 'content',
          lifecycle: { kind: 'create', mode: process.platform === 'win32' ? 0o400 : 0o640 },
        },
      ],
    }
  ...
  it('undoes and reapplies a deleted UTF-8 file', async () => {
    ...
    expect((await lstat(filename)).mode & 0o777).toBe(process.platform === 'win32' ? 0o666 : 0o600)
