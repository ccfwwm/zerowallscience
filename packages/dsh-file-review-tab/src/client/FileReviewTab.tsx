...
export function FileReviewTab({
  ...
  const uiConversation = binding?.ctx.get('uiConversation') as UiConversation | undefined
  const session = binding === undefined ? undefined : uiConversation?.binding(binding).target('chat')
  const getSessionSnapshot = useCallback(() => session?.getSnapshot() ?? EMPTY_SNAPSHOT, [session])
  const subscribeSession = useCallback(
    (listener: () => void) => (visible ? (session?.subscribe(listener) ?? (() => {})) : () => {}),
    [session, visible],
  )
