...
// The physical worker is unpacked, while its JS dependencies live in ASAR.
const runtimeRequire = createRequire(import.meta.url.replace('/app.asar.unpacked/', '/app.asar/'));
const { Jimp } = runtimeRequire('jimp');
...
async function getSharp() {
  if (_sharpMod === null) {
    try { const m = runtimeRequire('sharp'); _sharpMod = m.default || m; } catch (e) { _sharpMod = false; }
  }
...
async function main() {
  ...
  if (args.stdin) {
    ...
    if (config.normalize) {
      ...
      await sharp(config.normalize.input).rotate().png().toFile(config.normalize.output);
