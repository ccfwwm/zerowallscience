...
function toRis(p) {
    ...
    if (p.venue)
        lines.push(`JO  - ${p.venue}`);
    if (p.abstract)
        lines.push(`AB  - ${p.abstract.replace(/\s+/g, ' ')}`);
    if (p.doi)
        lines.push(`DO  - ${p.doi}`);
    if (p.url)
        lines.push(`UR  - ${p.url}`);
...
function toBib(p) {
    ...
    if (p.venue)
        lines.push(`  journal = {${esc(p.venue)}},`);
    if (p.doi)
        lines.push(`  doi = {${p.doi}},`);
    if (p.abstract)
        lines.push(`  abstract = {${esc(p.abstract.replace(/\s+/g, ' '))}},`);
    lines.push('}');
    return lines.join('\n') + '\n';
}
export function writeInbox(inboxDir, p, pdfBytes) {
