...
export const titleKey = (text) => text.normalize('NFKC').toLocaleLowerCase().replace(/[^\p{L}\p{N}]/gu, '');
export const doiKey = (text) => text.replace(/^https?:\/\/(?:dx\.)?doi.org\//i, '').replace(/^doi:\s*/i, '').trim().toLowerCase();
...
export class LocalZoteroWriter {
    ...
    get grantId() { return `${this.cfg.zoteroApiBase}|${this.serverId}`; }
    get credential() { return credentialKey('zotero-harvest', 'zotero-api-' + createHash('sha256').update(this.grantId).digest('hex')); }
    async connect() {
        const response = await fetch(this.cfg.zoteroApiBase + '/api/', { redirect: 'error', signal: AbortSignal.timeout(this.cfg.httpTimeoutMs) });
        if (!response.ok)
            throw new Error(`Zotero Local API unavailable (HTTP ${response.status}). Enable it in Zotero settings.`);
        this.serverId = response.headers.get('Zotero-Server-ID') ?? '';
        if (!this.serverId)
            throw new Error('This Zotero version does not expose authenticated local writes. Update Zotero or use inbox mode for manual import.');
    }
    async read(path) {
        ...
        if (!response.ok)
            throw new Error(`Zotero read failed (HTTP ${response.status})`);
        if (response.headers.get('Zotero-Server-ID') !== this.serverId)
            throw new Error('Zotero instance changed; operation stopped');
    ...
    async storedGrant() {
        ...
        const payload = stored?.kind === 'grant' ? stored.payload : null;
        if (payload?.serverId === this.serverId && typeof payload.key === 'string') {
            const grant = { key: payload.key, remember: true };
        ...
        if (/^[a-z]/.test(legacyId)) {
            ...
            const value = legacy?.kind === 'grant' ? legacy.payload : null;
            if (value?.serverId === this.serverId && typeof value.key === 'string' && value.key) {
                ...
                const grant = { key: value.key, remember: true };
                memories.set(this.grantId, grant);
                return grant;
            }
        }
    ...
    async authorize() {
        ...
        try {
            ...
            if (pendingAuthorizations.get(this.grantId) === pending)
                pendingAuthorizations.delete(this.grantId);
        }
    }
    async acquireGrant() {
        ...
        if (!response.ok)
            throw new Error(`Zotero write authorization was denied or unavailable (HTTP ${response.status})`);
        const grant = await response.json();
        if (typeof grant.key !== 'string' || !grant.key || response.headers.get('Zotero-Server-ID') !== this.serverId)
            throw new Error('Invalid Zotero authorization response');
        if (grant.remember)
            await this.cfg.credentials?.modifyRecord(this.credential, async () => ({ kind: 'grant', payload: { key: grant.key, serverId: this.serverId } }));
        memories.set(this.grantId, grant);
        return grant;
    }
    async invalidate() {
    ...
    async write(path, body, form = false, headers = {}) {
        for (let attempt = 0; attempt < 2; attempt++) {
            ...
            if (!response.ok)
                throw new Error(`Zotero write failed (HTTP ${response.status}); no automatic replay was attempted`);
            if (response.headers.get('Zotero-Server-ID') !== this.serverId)
                throw new Error('Zotero instance changed during write');
    ...
    async find(paper) {
        ...
        for (const q of queries) {
            ...
            const rows = await this.read('/api/users/0/items?' + query);
    ...
    async collection(name) {
        ...
        for (let start = 0; start < 10000; start += 100) {
            const rows = await this.read(`/api/users/0/collections?limit=100&start=${start}`);
            matches.push(...rows.filter(row => row.key === name || row.data.name === name));
            if (rows.length < 100)
                break;
        }
        if (matches.length > 1)
            throw new Error('Multiple Zotero collections have this name; supply the collection key');
        if (matches[0])
            return matches[0].key;
        const key = keyOf();
        return this.created(await this.write('/api/users/0/collections', [{ key, name, parentCollection: false }]), key).key;
    }
    ref(key) { return `zotero://user/0/item/${key}?server=${encodeURIComponent(this.serverId)}`; }
    async add(p, collection) {
        const key = keyOf();
    ...
    async attach(parentKey, bytes) {
        ...
        if (!upload.exists) {
            ...
            if (url.origin !== this.cfg.zoteroApiBase || !/^\/api\/local\/uploads\/[A-Za-z0-9]+$/.test(url.pathname))
                throw new Error('Zotero returned an unexpected upload destination');
