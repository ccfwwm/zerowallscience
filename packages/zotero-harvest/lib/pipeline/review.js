...
function markdownReport(topic, subtopics, rounds, collected) {
    ...
    for (const r of rounds) {
        lines.push(`- Round ${r.round}: queries=${r.queries.join(' | ') || '—'} fetched=${r.fetched} new=${r.newPapers} core=${r.coreCount} total=${r.totalCount} sufficient=${r.sufficient}`);
    }
    lines.push('');
    lines.push('## Collected papers');
    collected.forEach((p, i) => {
        lines.push(`${i + 1}. **${p.title}** (${p.year ?? '—'}) — ${p.authors.slice(0, 3).join(', ') || '—'} [${p.source}:${p.id}]${p.doi ? ` DOI:${p.doi}` : ''}`);
    });
    return lines.join('\n');
}
