...
export async function httpGetJson(url, headers = {}, timeoutMs = REQUEST_TIMEOUT_MS) {
...
export async function openalexSearch(query, max, opts = {}) {
    ...
    const data = (await httpGetJson(`https://api.openalex.org/works?${params}`));
    if (!data.results)
        return [];
    const out = [];
...
export async function arxivSearch(query, max) {
    ...
    const xml = await fetch(`https://export.arxiv.org/api/query?${q}`, {
        ...
        if (!r.ok)
            throw new Error(`HTTP ${r.status} for arXiv query`);
        return r.text();
    });
    const out = [];
...
/* ── Crossref (keyless) ────────────────────────────────────────────────── */
export async function crossrefSearch(query, max) {
    const params = new URLSearchParams({
        query: query,
        rows: String(max),
        'mailto': 'lit-harvest@local',
    ...
    const data = (await httpGetJson(`https://api.crossref.org/works?${params}`));
...
export async function semanticScholarSearch(query, max, apiKey) {
    const params = new URLSearchParams({
        query,
        limit: String(max),
    ...
    const data = (await httpGetJson(`https://api.semanticscholar.org/graph/v1/paper/search?${params}`, headers));
...
export async function europepmcSearch(query, max) {
    const params = new URLSearchParams({
        query,
        format: 'json',
        pageSize: String(max),
    ...
    const data = (await httpGetJson(`https://www.ebi.ac.uk/europepmc/webservices/rest/search?${params}`));
...
export async function unpaywallForDoi(doi, email) {
    const data = (await httpGetJson(`https://api.unpaywall.org/v2/${encodeURIComponent(doi)}?email=${encodeURIComponent(email)}`));
    const pdfUrls = [...new Set((data.oa_locations ?? [])
            .map((l) => l.url_for_pdf)
            .filter((u) => !!u))];
    const landingUrls = [...new Set((data.oa_locations ?? [])
            .map((l) => l.url)
            .filter((u) => !!u))];
    return {
 ...
 */
export async function scholarSearch(query, max, proxy) {
...
/* ── dispatch ──────────────────────────────────────────────────────────── */
export async function fetchBySource(source, query, max, opts) {
    switch (source) {
        case 'openalex':
