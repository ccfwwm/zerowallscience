...
import { resolveConfig, withConfig } from './config.ts'
...
import { LocalZoteroWriter } from './save/local-api.ts'

/** Settings and tool calls share the same Host credential provider; never return secrets. */
export async function localAuthorization(ctx: CtxLike, action: 'status' | 'authorize' | 'renew') {
...
export function apply(ctx: CtxLike): void {
  ...
  ctx.tools.register(defineTool({
    name: 'lit_fetch',
    description:
      'Fetch academic papers for a query from multiple sources (OpenAlex, arXiv, Crossref, Europe PMC, optional Semantic Scholar), normalize, dedupe by DOI/title, rank, and resolve open-access download links (Unpaywall) when an open-access PDF is available. A DOI or arXiv id as the query returns that exact paper.',
    parameters: {
    ...
    timeoutMs: 180_000,
    execute: async (args: Record<string, unknown>) => withConfig(currentConfig(), async () => {
  ...
  ctx.tools.register(defineTool({
    name: 'lit_save',
    description:
      'Save papers into the local Zotero library. Mode auto: uses the Zotero local API when the desktop is running, otherwise prepares manual-import RIS/BibTeX + PDFs into the inbox directory. Deduplicates by DOI/title against the library.',
    parameters: {
      papers: {
        type: 'array',
        items: paperSchema,
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args: Record<string, unknown>) => withConfig(currentConfig(), async () => {
      ...
      return runReview({
        ...
        saveMode: args.save_mode as SaveResult['mode'],
        collection: typeof args.collection === 'string' ? args.collection : undefined,
