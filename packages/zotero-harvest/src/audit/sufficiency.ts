...
function keywordsOf(text: string | undefined): Set<string> {
  ...
  for (const raw of text.toLowerCase().split(/[^a-z0-9\u4e00-\u9fff]+/)) {
    ...
    if (/^[a-z]+$/.test(w) && STOPWORDS.has(w)) continue
    out.add(w)
  }
  return out
}

function subtopicMatchesPaper(subtopic: string, p: Paper): boolean {
...
export function checkSufficiency(input: SufficiencyInput): SufficiencyReport {
  ...
  if (coreCount < minCore) reasons.push(`core papers ${coreCount}/${minCore}`)
  if (totalCount < minTotal) reasons.push(`total papers ${totalCount}/${minTotal}`)
  if (gaps.length > 0) reasons.push(`uncovered subtopics: ${gaps.join('; ')}`)
...
export function decomposeSubtopics(topic: string, explicit?: string[]): string[] {
  if (explicit && explicit.length > 0) return explicit.map((s) => s.trim()).filter(Boolean)
