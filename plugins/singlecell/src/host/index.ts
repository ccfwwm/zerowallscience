...
export class ZeroWallSinglecellService extends TypertRemoteService {
  ...
  constructor(ctx: Context) {
    ...
    register({ name: 'sc_tenifold_knockout_validate', description: '校验 scTenifoldKnk 输入和研究元数据。', parameters: { config: { type: 'object', additionalProperties: true, required: true } }, output, execute: async (args: any, exec: any) => this.scTenifoldValidate({ sessionId: String(exec.agent?.session.id ?? ''), config: args.config as ScTenifoldProjectConfig }) as unknown as Record<string, JsonValue> })
    register({ name: 'sc_tenifold_knockout_plan', description: '创建可审计的 scTenifoldKnk 分析计划。', parameters: { config: { type: 'object', additionalProperties: true, required: true } }, output, execute: async (args: any, exec: any) => this.scTenifoldPlan({ sessionId: String(exec.agent?.session.id ?? ''), config: args.config as ScTenifoldProjectConfig }) as unknown as Record<string, JsonValue> })
    register({ name: 'sc_tenifold_knockout_run', description: '运行或提交 scTenifoldKnk 虚拟敲除。', parameters: { config: { type: 'object', additionalProperties: true, required: true }, target: { type: 'string' } }, output, execute: async (args: any, exec: any) => this.runInternal({ sessionId: String(exec.agent?.session.id ?? ''), config: args.config as ScTenifoldProjectConfig, ...(args.target ? { target: String(args.target) } : {}) }, exec) as unknown as Record<string, JsonValue> })
...
export async function apply(ctx: Context): Promise<void> { await ctx.plugin(ZeroWallSinglecellService) }
