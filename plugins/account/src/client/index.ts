...
export function apply(ctx: ClientContext): void {
  ...
  ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
    ...
    inject: () => ({
      ...
      register: async (email: string, password: string, verificationCode: string, rememberPassword: boolean) => unwrapRemoteResult('zerowall.account.register', await remote.zerowallAccount.register({ email, password, verificationCode, rememberPassword })),
