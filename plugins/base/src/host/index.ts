...
export const SCIENCE_SYSTEM_PROMPT_VERSION = '7.0.4-core.1'
...
export function apply(ctx: Context): void {
  ctx.systemPrompt.section({ name: 'zerowall:shared-python', order: 91, text: 'ZeroWall Science Shared Python Runtime belongs to the whole application; MCP is one consumer. Use python_environment to inspect status, check signed manifests, preview dependencies, and diagnose failures. Apply or rollback only after the user approves the concrete plan. Never directly modify site-packages or invent computational results. Installation and TLS failures must include the returned diagnostic and repair entry. Remote Linux environments are separate and require platform-specific manifests.' })
