...
describe('genetic analysis applicability checks', () => {
  ...
  it('accepts a complete multi-instrument MR contract', () => expect(validateGeneticContract(valid)).toMatchObject({ status: 'usable', contract: '7.0.0-genetic-contract.1' }))
  it('rejects a lead-SNP-only coloc request and refuses unsupported Egger', () => {
    ...
    expect((egger.findings as any[]).map(item => item.code)).toContain('egger-insufficient-instruments')
  })
