...
it('selects a registered image, runs segmentation, shows masks and reuses the retry key', async () => {
  ...
  expect(fijiExperiment.mock.calls[0]?.[0]).toMatchObject({ sourceAssetId: 'a1', image: { kind: 'scratch-wound' } })
  await waitFor(() => expect(screen.getByRole('button', { name: '分析图像' })).toBeTruthy())
  fireEvent.click(screen.getByRole('button', { name: '分析图像' }))
  await waitFor(() => expect(fijiExperiment).toHaveBeenCalledTimes(2))
  expect((fijiExperiment.mock.calls[1] as any)[0].requestId).toBe((fijiExperiment.mock.calls[0] as any)[0].requestId)
})
