...
export const GENETIC_CONTRACT_VERSION = '7.0.0-genetic-contract.2'
...
export function validateGeneticContract(input: JsonObject): JsonObject {
  ...
  if (['mr', 'ivw', 'wald-ratio', 'mr-egger', 'mvmr', 'two-step-mr'].includes(method)) {
    ...
    if (method === 'wald-ratio' && instruments !== undefined && instruments !== 1) { add('wald-ratio-tool-count', 'Wald ratio 只适用于单个有效工具；多工具需按冻结方案选择合并方法。'); notApplicable.add('wald-ratio-tool-count') }
    if (method === 'mr-egger' && instruments !== undefined && instruments < 3) { add('egger-insufficient-instruments', 'MR-Egger 至少需要适用的多个工具；单工具或两个工具不得自动运行。'); notApplicable.add('egger-insufficient-instruments') }
    if (method === 'ivw' && instruments !== undefined && instruments < 2) { add('ivw-insufficient-instruments', 'IVW 不能把单个工具当作多工具合并；请改用适用的 Wald ratio 或停止。'); notApplicable.add('ivw-insufficient-instruments') }
    if (method === 'mr' && instruments === 1) { add('single-instrument-mr', '单工具 MR 必须明确采用 Wald ratio；不能按通用多工具 MR 执行。'); notApplicable.add('single-instrument-mr') }
  ...
  if (method === 'mvmr') {
    const conditionalF = number(input.conditionalFStatistic)
    if (conditionalF === undefined) missing.push('conditionalFStatistic')
    else if (conditionalF <= 0) errors.push('conditionalFStatistic must be positive.')
  }
  if (method === 'two-step-mr' && !text(input.mediationEstimand)) missing.push('mediationEstimand')
  if (input.sampleOverlap === 'unknown') add('sample-overlap-unknown', '样本重叠未知时只能登记限制，不能把 MR 结果写成无偏因果证据。')
  const status = errors.length || findings.some(item => item.code === 'lead-snp-only') || findings.some(item => notApplicable.has(String(item.code))) ? 'not-applicable' : missing.length ? 'pending' : 'usable'
