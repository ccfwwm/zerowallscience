import { applySangerEdits } from '../shared/sanger-revision.js'
...
export class SangerService {
  ...
  async execute(project: ProjectRecord, request: SangerRequest): Promise<SangerResponse> {
    ...
    if (!['analyze', 'export', 'review', 'revise'].includes(request.action)) throw new Error('Unsupported Sanger action.')
    ...
    if (request.action === 'review') {
      ...
      if (forwardInput.sha256 === reverseInput.sha256) throw new Error('Bidirectional review requires distinct source traces, not two views of the same file.')
      ...
      const currentViews = this.store.listViewerSessions(project.id)
      if (currentViews.find(v => v.id === viewer.id)?.version !== viewer.version || currentViews.find(v => v.id === reverse.id)?.version !== reverse.version) throw new Error('Sanger review revision conflict during source reading; refresh both views.')
      const forward = analyzeSanger(applySangerEdits(forwardInput.trace, viewer.state.editHistory), request.threshold ?? Number(viewer.state.threshold), request.window ?? Number(viewer.state.window), request.reference)
      const reverseAnalysis = analyzeSanger(applySangerEdits(reverseInput.trace, reverse.state.editHistory), request.threshold ?? Number(reverse.state.threshold), request.window ?? Number(reverse.state.window))
      return { trace: applySangerEdits(forwardInput.trace, viewer.state.editHistory), analysis: forward, review: reviewBidirectionalSanger(forward, reverseAnalysis), viewer }
    }
    ...
    if (request.action === 'revise') {
      const history = Array.isArray(viewer.state.editHistory) ? viewer.state.editHistory : []
      const batch = { sourceSha256: input.sha256, edits: request.edits, createdAt: new Date().toISOString(), baseViewerVersion: viewer.version }
      const revised = applySangerEdits(input.trace, [...history, batch])
      const updated = this.store.updateViewerSession(project.id, viewer.id, { expectedVersion: viewer.version, invalidateOutputs: true, state: { ...viewer.state, threshold: 0, editHistory: JSON.parse(JSON.stringify([...history, batch])) } })
      return { trace: revised, analysis: analyzeSanger(revised, 0, Number(viewer.state.window)), viewer: updated }
    }
    input.trace = applySangerEdits(input.trace, viewer.state.editHistory)
    const threshold = request.threshold ?? Number(viewer.state.threshold); const window = request.window ?? Number(viewer.state.window); const analysis = analyzeSanger(input.trace, threshold, window, request.reference); const updated = this.store.updateViewerSession(project.id, viewer.id, { expectedVersion: viewer.version, state: { ...viewer.state, sourceSha256: input.sha256, traceTool: 'sanger', threshold, window, ...(request.reference === undefined ? {} : { reference: request.reference }) } }); if (request.action === 'analyze') return { trace: input.trace, analysis, viewer: updated }; if (request.action !== 'export') throw new Error('Unsupported Sanger action.')
    const root = await realpath(project.rootPath); const parentPath = join(root, '.zerowall'); await mkdir(parentPath, { recursive: true }); const parent = await containedFile(root, parentPath); const directoryPath = join(parent, 'science-exports'); await mkdir(directoryPath, { recursive: true }); const directory = await containedFile(root, directoryPath); const destinationPath = join(directory, randomUUID()); await mkdir(destinationPath); const destination = await containedFile(root, destinationPath); const manifest = JSON.stringify({ format: 'zerowall-sanger-result', version: 1, runner: 'zerowall-sanger/7.0.0-2', sourceAssetId: asset.id, sourceSha256: input.sha256, viewerId: viewer.id, viewerVersion: updated.version, editHistory: viewer.state.editHistory ?? [], parameters: { threshold, window, referenceProvided: request.reference !== undefined }, analysis }, null, 2) + '\n'; const path = join(destination, 'result.json')
    try { await writeFile(path, manifest, { flag: 'wx' }); await writeFile(join(destination, 'trimmed.fasta'), `>trimmed source=${input.sha256}\n${analysis.trim.sequence}\n`, { flag: 'wx' }); const artifact = this.store.createArtifacts([{ projectId: project.id, name: 'Sanger trace analysis', uri: pathToFileURL(path).href, mediaType: 'application/json', checksum: createHash('sha256').update(manifest).digest('hex'), metadata: { sourceAssetId: asset.id, sourceSha256: input.sha256, viewerId: viewer.id, viewerVersion: updated.version, runner: 'zerowall-sanger/7.0.0-2', fastaUri: pathToFileURL(join(destination, 'trimmed.fasta')).href, needsReview: Array.isArray(viewer.state.editHistory) && viewer.state.editHistory.length > 0 } }], { projectId: project.id, viewer: { id: viewer.id, version: updated.version } })[0]!; return { trace: input.trace, analysis, viewer: updated, artifact } } catch (error) { await rm(destination, { recursive: true, force: true }); throw error }
  }
