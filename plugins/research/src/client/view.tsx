...
type ResearchTask = { id: string; name: string; kind: string; status: string; dependencies: string[]; attempt: number; exploratory: boolean; error?: string }
...
const tools: Tool[] = [
  ...
  { id: 'he', name: 'HE 查看器', formats: 'SVS · NDPI · TIFF · ROI RGB/核样本统计（StarDist 待接入）', location: '内置首层 + CPU 受限 ROI', state: 'partial' },
  { id: 'structure', name: '分子结构', formats: 'PDB · mmCIF · SDF · 对接', location: 'Mol* + rdatalinux', state: 'partial' },
...
export function ScienceWorkbench(props: TabComponentProps & { remote: Remote }): JSX.Element {
  ...
  const load = async (nextStudyId?: string): Promise<void> => {
    ...
    try {
      ...
      if (!project?.id) throw new Error('当前会话没有匹配的 ZeroWall 项目，请先打开项目工作区。')
      ...
      setProjectId(project.id); setStudies(listed); setSnapshot(data); setSelectedStudy(id)
  ...
  useEffect(() => {
    setProjectId(undefined); setStudies([]); setSnapshot(undefined); setSelectedStudy(undefined); setRationale('')
    void load()
  ...
  return <div style={styles.root}>
    ...
    {page === 'data' && <DataPage active={active} snapshot={snapshot} documentKind={documentKind} setDocumentKind={setDocumentKind} documentText={documentText} setDocumentText={setDocumentText} createDocument={() => void createDocument()} />}
