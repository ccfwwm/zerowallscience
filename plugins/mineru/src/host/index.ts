...
export class ZeroWallMineruService extends TypertRemoteService {
  ...
  @Remote('testAgent') async testAgent(): Promise<{ ok: true; api: 'agent' }> {
    ...
    try {
      const response = await fetch(`${this.config().apiBaseUrl.replace(/\/+$/u, '')}/api/v1/agent/parse`, { method: 'GET', headers: { ...(token ? { authorization: `Bearer ${token}` } : {}), accept: 'application/json' }, signal: controller.signal })
      if (!response.ok) throw new Error(`MinerU Agent API 测试失败（HTTP ${response.status}）。`)
