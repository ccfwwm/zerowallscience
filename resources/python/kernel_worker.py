#!/usr/bin/env python3
"""ZeroWall kernel worker — persistent Python execution over a JSON-per-line
...
This is a Windows-friendly port of the upstream zerowall-science
...
def _neutralize_pyplot_show() -> None:
    ...
    if show is None or getattr(show, "_zerowall_noop", False):
        return

    def _noop_show(*_a, **_k):  # ponytail: figures go to savefig, not a GUI
        return None

    _noop_show._zerowall_noop = True
    plt.show = _noop_show
...
def main():
    ...
    while True:
        ...
        if code_size > MAX_CODE_SIZE or code_lines > MAX_CODE_LINES:
            ...
            continue
        cell_counter += 1
        cell_tag = f"<zerowall-kernel:{cell_counter}>"
