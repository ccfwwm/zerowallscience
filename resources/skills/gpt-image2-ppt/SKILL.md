...
## ZeroWall 默认生图路径（必须优先）

当 ZeroWall 提供内置 `generate_image` / `edit_image` 工具，且用户没有显式提供另一套 API Key 和地址时，必须自动使用这些工具。它们会从当前已登录的 AI Cloud 配置中精确选择 `gpt-image-2`，凭据保留在 Host 的系统加密存储中；不要扫描 `.env`、Shell 环境、Codex/Claude 配置，也不要要求用户重复提供 Key。

固定流程：

1. 按下文生成 `slides_plan.json`，再运行 `generate_ppt.py --prepare-only` 得到 `prompts.json`。
2. 逐页读取 `prompts.json`：没有 reference 的新页面调用 `generate_image`；存在 `reference_image` / `asset_reference_image` 的严格模板复刻、参考图生成调用 `edit_image`，并按记录顺序传入 `input_paths`。输出到 `outputs/<timestamp>/images/slide-NN.png`。普通横版页面使用 `size=1536x1024`，默认模型参数省略。
3. 所有页面图片完成后，再运行同一个 `generate_ppt.py` 命令；脚本会跳过已经存在的 PNG，并完成 PPTX 打包及真实图片叠加。

`gpt-image-2` 禁止发送到 `/messages` 或 `/chat/completions`。内置路径失败时，应原样报告“未登录 / 未发现模型 / 分组凭据缺失 / Image API 状态”，不得改走聊天协议。只有用户明确提供独立 Key、地址并要求 API 直连时，才使用下面的 Python 直连配置。

ZeroWall 内置工具的固定路由：全新页面且无参考图用 `generate_image`；模板严格复刻、已有单页修改、外部图片槽位二次修复、局部 repair mask 或任何保留现有构图的流程用 `edit_image`。单页修改时当前 `images/slide-NN.png` 必须作为第一张 `input_paths`；模板页、asset skeleton 等补充 reference 按 `prompts.json` 顺序追加。没有可确认的当前页或存在多个 session 候选时先询问，不得新生成一页冒充编辑结果。
...
> **安全提示**：这一节仅适用于用户明确要求的独立 API 直连。ZeroWall 默认路径不读取这些变量，而是调用 Host `generate_image`。直连脚本只读取当前进程环境、平台注入的 `gpt-image2-ppt_*` 变量、显式 `GPT_IMAGE2_PPT_ENV`，以及 skill 安装目录下的 `.env` fallback；不会向上递归读取调用者项目目录里的 `.env`。

## 如果当前会话提供 ZeroWall Host 图片工具（推荐）

当前 ZeroWall 会话若提供 `generate_image` / `edit_image`，不要用 `generate_ppt.py` 或 `--backend codex` 负责出图：无 reference 的新页调用 `generate_image`，有 reference 的模板复刻、单页修改和槽位修复调用 `edit_image`，最后只复用本仓库的 md 转换 / PPTX 打包逻辑。

关键边界：Python 脚本运行在子进程里，拿不到当前会话里的 Host 工具。`generate_ppt.py --backend codex` 能做的只有再启动一个 `codex exec` 子进程；它不会复用当前会话的图片工具，也不应绕过 Host 凭据与审计。

### 如何判断

你能访问 `generate_image` / `edit_image` 且不需要手动配 `OPENAI_API_KEY` 就能出图或编辑——满足条件就走 Host 路径。若当前会话没有这些工具，才按普通 agent 处理：走用户明确授权的 API 直连或 `--backend codex` 备用后端。
...
**4. 按 reference 路由调用 ZeroWall 图片工具出图**

对每页调用 `generate_image` 或 `edit_image`：

- `prompt`: `prompts.json` 中该页已经编译好的完整 prompt
- 没有 `reference_image` / `asset_reference_image` 时调用 `generate_image`
- 记录了 reference 时调用 `edit_image`，当前页或模板页放入 `input_paths`，按原顺序追加其它 reference；模板严格复刻、已有页修改、外部槽位修复不得改用新生成
- `output_path`: `outputs/<timestamp>/images/slide-NN.png`
- `size`: 普通横版 `1536x1024`
...
| | ZeroWall Host 路径（本节） | --backend codex |
|---|---|---|
| 适用场景 | 当前 ZeroWall 会话可用 Host 图片工具 | 你是 Claude Code / 其他 agent，借用本机 codex CLI |
| 调用方式 | 无 reference 用 `generate_image`，有 reference 用 `edit_image` | spawn `codex exec --full-auto` 子进程 |
...
5. **出图冒烟**：
   ...
   - 当前会话带 ZeroWall Host 图片工具：按上方 Host 路径调用 `generate_image` / `edit_image` 生成 `outputs/<timestamp>/images/slide-01.png`；不要用 `generate_ppt.py --backend codex`
