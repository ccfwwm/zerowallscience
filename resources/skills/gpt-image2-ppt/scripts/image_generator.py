...
默认调用 OpenAI 兼容的 /v1/images/generations 端点。
只有用户显式设置 GPT_IMAGE_ENDPOINT=chat 时才启用旧式中转站聊天协议；
images/auto 模式绝不把失败的 gpt-image-2 请求回退到聊天端点。
"""
...
class GptImage2Generator:
    ...
    def __init__(self, aspect_ratio: str = "16:9") -> None:
        ...
        # endpoint: images | auto | chat。auto 是 images 的兼容别名，不再回退聊天协议。
        self.endpoint = os.getenv("GPT_IMAGE_ENDPOINT", "images").lower()
        if self.endpoint not in ("images", "auto", "chat"):
            raise ValueError("GPT_IMAGE_ENDPOINT 只能是 images、auto 或显式 chat")
    ...
    def generate_scene_image(
        ...
        for ratio_attempt in range(MAX_ASPECT_RETRIES + 1):
            ...
            for attempt in range(1, MAX_RETRIES + 1):
                try:
                    if self.endpoint in ("images", "auto"):
                        # 禁止隐式回退聊天协议；参考图需走 ZeroWall Host 工具或显式 chat。
                        if reference_image_path:
                            raise ValueError(
                                "Images API 直连暂不接受本脚本的参考图；请使用 ZeroWall "
                                "generate_image 工具，或由用户显式设置 GPT_IMAGE_ENDPOINT=chat。"
                            )
