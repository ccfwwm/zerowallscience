...
def _try_powerpoint_render(pptx_path: Path, out_dir: Path) -> Optional[int]:
    ...
    try:
        ...
        exported_pages = _collect_powerpoint_export_pages(out_dir, count)
        if exported_pages is None:
            names = ", ".join(path.name for path in sorted(out_dir.glob("*.PNG")))
            raise RuntimeError(
                "PowerPoint exported slides with unrecognized filenames: "
                f"expected {count} numbered PNGs, found [{names}]"
            )
        for page_num, source in exported_pages:
...
def _collect_powerpoint_export_pages(
    out_dir: Path,
    slide_count: int,
) -> Optional[list[tuple[int, Path]]]:
    """Collect localized PowerPoint exports such as Slide1.PNG or 幻灯片1.PNG."""
    numbered: dict[int, Path] = {}
    for path in out_dir.iterdir():
        if not path.is_file() or path.suffix.lower() != ".png":
            continue
        if re.fullmatch(r"page-\d+", path.stem, flags=re.IGNORECASE):
            continue
        match = re.search(r"(\d+)$", path.stem)
        if match is None:
            continue
        page_num = int(match.group(1))
        if page_num < 1 or page_num > slide_count or page_num in numbered:
            return None
        numbered[page_num] = path
    if set(numbered) != set(range(1, slide_count + 1)):
        return None
