...
zerowall-science needs three **independent** local toolchains:
...
Restart zerowall-science after changing PATH or global config so bootstrap re-runs.
...
zerowall-science does **not** ship Python. It needs **`uv`** on PATH (or `UV_PATH`) to create the managed venv.

### What gets created automatically

1. `uv venv` → virtualenv under app data
2. `uv pip install -r …/python/requirements-mcp.txt`
3. Marker `.zerowall_deps_ok` when deps succeed

| OS | Desktop venv path |
|---|---|
| Windows | `%APPDATA%\science.zerowall-science\zerowall-science\python\.venv` |
| macOS | `~/Library/Application Support/science.zerowall-science/zerowall-science/python/.venv` |
| Linux | `~/.local/share/science.zerowall-science/zerowall-science/python/.venv` |

Dev checkout: `<workspace>/.zerowall/python/.venv`
...
In the zerowall-science desktop app, you can also save the SciMaster key in
Settings -> Credentials -> SCIMaster. ZeroWall will sync that key into
`~/.scimaster/config.json` for `scimaster-cli`.

## Layer 3 — Bioinformatics: pixi

**pixi** manages isolated per-project environments (conda + pip) — use for scanpy/single-cell, variant calling stacks, etc. The zerowall **`python` tool** uses the core uv venv; run bioinfo code via **`shell`**: `pixi run python …` or `pixi run …` in the project directory.
...
| Issue | Fix |
|---|---|
| uv/node installed but app still says missing | Restart zerowall-science; confirm tools on PATH for the **GUI user** (macOS: relaunch from Dock after shell profile update). |
| Cannot modify PATH | Set `UV_PATH` / `PIXI_PATH` to full binary paths before launching zerowall-science. |
...
5. Verify each layer; tell user to **restart zerowall-science** after PATH/config changes.
...
- Remote GPU or direct SSH → `compute-env-setup`; managed cloud backends are
  unavailable until ZeroWall implements a matching execution-context backend
