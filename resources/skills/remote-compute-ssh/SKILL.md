...
Use this skill after choosing an `ssh:<alias>` execution context. ZeroWall owns the
...
3. To watch the Run or wait for later work, call `monitor_run` exactly once with
   the returned Run id. ZeroWall inserts a live card in the conversation, suspends
...
error is stored as `last_poll_error`; do not resubmit, because ZeroWall retries the
...
`input_paths` are project-relative local files. ZeroWall validates them, copies
...
The control directory is `~/.zerowall-science/runs/<run-id>` and the command starts
...
running without a wall-time limit. ZeroWall maps the supervisor timeout marker to
`timed_out`.
...
ZeroWall rejects globs, symlinks, special files, and existing remote destinations.
...
ZeroWall stages the item beside the destination, never overwrites an existing
...
It creates a dedicated key on A, carries only the public key through ZeroWall,
...
want server SSH configuration changed, select the relay route; ZeroWall downloads
to a private local temporary directory and uploads with B's separately stored
credentials.

## Cancellation and recovery

`cancel_run({"run_id":"..."})` changes an SSH Run to `cancelling`. ZeroWall
