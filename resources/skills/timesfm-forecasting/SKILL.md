...
> download on-demand from HuggingFace on first use and cache under
> `Path.home() / ".cache" / "huggingface"` unless `HF_HOME` is configured.
...
### With `matplotlib` / `publication-figures`
