...
def _resolve_api_key(explicit: Optional[str] = None) -> Optional[str]:
    """Resolve an explicit in-memory key or ZeroWall-injected environment key."""
    if explicit:
        return explicit
...
"""
    ...
    def __init__(self, api_key: Optional[str] = None, verbose: bool = False):
        ...
        """
        # The CLI uses only the ZeroWall-injected environment variable. Library
        # callers may pass an in-memory key without persisting it.
        self.api_key = _resolve_api_key(api_key)

        if not self.api_key:
            raise ValueError(
                "OPENROUTER_API_KEY not found. Please either:\n"
                "  1. Configure OPENROUTER_API_KEY in ZeroWall Settings > Credentials\n"
                "  2. Pass an in-memory api_key parameter to the constructor\n"
...
Environment:
  OPENROUTER_API_KEY    OpenRouter API key (required)
    ...
    parser.add_argument("--execute", action="store_true",
                       help="Perform the billable external generation request")
    ...
    if not args.execute:
        print("Dry run: no external request was sent.")
        print(json.dumps({
            "prompt": args.prompt,
            "output": args.output,
        ...
        print("Re-run with --execute only after the user approves the external call.")
        return

    api_key = _resolve_api_key()
    if not api_key:
        print("Error: OPENROUTER_API_KEY not found")
        print("\nConfigure OPENROUTER_API_KEY in ZeroWall Settings > Credentials.")
        sys.exit(1)
