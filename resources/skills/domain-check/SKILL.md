...
zerowall:
  ...
  roles: [analyst, critic, validator]
...
After `use_skill`, run the absolute `domain_check.py` path listed under
`Bundled Resources`:

```text
python "<absolute path to domain_check.py>" <file.py|notebook.ipynb|analysis.R ...>
```
