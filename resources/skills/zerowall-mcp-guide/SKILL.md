...
Use the smallest suitable MCP family. Biomedical analysis and Biomni agent work use `mcp__rmcp__rbioagent__*`; general R projects, computation, and workspace uploads use `mcp__rmcp__rplatform__*`; FigureYa plotting, reproducible figures, manifests, asynchronous jobs, and reports use `mcp__rmcp__rplotfigure__*`. Chemical searches, compound details, reactions, structures, and stoichiometry use the AIchem MCP. Scientific writing uses Sci, biological database operations use Bio Tools, and molecular drawing uses Ketcher Chemistry.
