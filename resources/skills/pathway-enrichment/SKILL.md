...
For quick one-off Enrichr lookups the `gget` skill (`gget enrichr`) is lighter weight; for raw pathway/interaction APIs (Reactome, KEGG, STRING) see the `search_mcp_tools` skill. Use **this** skill for full, defensible enrichment workflows.
...
- **Databases / IDs:** `search_mcp_tools` (Reactome, KEGG, STRING, Gene Ontology APIs), `gget` (`gget enrichr` quick path, `gget info` for ID mapping), `bioservices`.
- **Downstream:** `publication-figures` (custom figures), `networkx` (enrichment-map graphs), `paper-to-report` / `literature-review` (interpret and cite), `statistical-analysis` (multiple-testing details).
