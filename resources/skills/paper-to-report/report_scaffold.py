...
Collects (stdlib only, no network):
  data     - data files and size; stored provenance is queried by ZeroWall
  methods  - scripts, environment manifest, pinning
  results  - one subsection per figure, with its generator and inputs
  repro    - rebuild commands and figures with no generator
...
def collect(root: str, title: str) -> Inventory:
    ...
    inv.data = [
        {"path": f, "size": _size(root, f), "provenance": "unknown"}
        for f in files
...
def section_data(inv: Inventory) -> list[str]:
    ...
    for d in inv.data:
        out.append(f"| `{d['path']}` | {d['size']} | unknown; query ZeroWall |")
    out += [""]
    out += [_todo("Call `inspect_project_provenance` for the data paths above and "
                  "replace each unknown status with the returned fact. Keep unknown "
                  "when stored evidence is insufficient."), ""]
    return out
...
def section_repro(inv: Inventory) -> list[str]:
    ...
    if generators:
        ...
        out += ["```", ""]
    out.append("- Stored provenance: **unknown** until `inspect_project_provenance` "
               "checks the project-relative paths below.")
    if orphans:
...
def section_open(inv: Inventory) -> list[str]:
    ...
    if inv.data or inv.figures:
        gaps.append("stored provenance not yet queried")
...
def as_json(inv: Inventory) -> dict:
    return {
        ...
        "provenance_paths": sorted(
            {d["path"] for d in inv.data}
            | {f.path for f in inv.figures}
            | {f.generator for f in inv.figures if f.generator}
        ),
        "figures": [
...
def main(argv: list[str]) -> int:
    ...
    root_path = Path(positional[0] if positional else os.getcwd()).resolve()
    root = str(root_path)
