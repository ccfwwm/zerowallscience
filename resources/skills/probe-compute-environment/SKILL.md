...
Use ZeroWall's environment Probe action. It performs bounded, read-only checks and
...
ssh-agent key must configure its `IdentityFile` in ZeroWall or SSH config.
