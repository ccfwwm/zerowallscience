...
description: Historical scientific environment recipes translated into package-order, cache, and validation examples for ZeroWall direct execution contexts. Read alongside compute-env-setup when building a matching user-space environment on SSH.
---

# Compute environment reference — worked examples

These are recipe examples, not environments ZeroWall automatically provides. Translate
