...
1. 默认 PDF、文档与文献 OCR 解析均使用本技能。先复用附件已有的 MinerU 结果，避免重复提交；需要新解析时调用 `mineru_activate` 后使用 `mineru_parse` 的 `precision` 模式，以取得 Markdown、图片和结构化定位。缺 Token 或失败时明确说明，不把本地快速读取称为已完成 OCR；用户接受降级后才使用本地提取。
