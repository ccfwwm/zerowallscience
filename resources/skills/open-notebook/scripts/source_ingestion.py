...
def add_url_source(notebook_id, url, process_async=True, *, execute=False):
...
def add_text_source(notebook_id, title, text, *, execute=False):
...
def upload_file_source(notebook_id, file_path, process_async=True, *, execute=False):
...
def retry_failed_source(source_id, *, execute=False):
...
def delete_source(source_id, *, execute=False):
    """Delete a source."""
    _require_execute(execute)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Run the external ingestion demo after explicit user approval",
    )
    args = parser.parse_args()
    if not args.execute:
        print("Dry run: source ingestion and deletion requests were not sent.")
        raise SystemExit(0)
    ...
    # Clean up
    print()
    delete_source(url_source["id"], execute=True)
    delete_source(text_source["id"], execute=True)
