...
def create_chat_session(notebook_id, title, model_override=None, *, execute=False):
...
def send_chat_message(session_id, message, include_sources=True,
                      include_notes=True, model_override=None, *, execute=False):
...
def build_context(notebook_id, source_ids=None, note_ids=None, *, execute=False):
...
def search_knowledge_base(query, search_type="vector", limit=5, *, execute=False):
...
def ask_question(query, *, execute=False):
...
def delete_chat_session(session_id, *, execute=False):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Run the external chat demo after explicit user approval",
    )
    args = parser.parse_args()
    if not args.execute:
        print("Dry run: chat, search, and deletion requests were not sent.")
        raise SystemExit(0)
    ...
    session = create_chat_session(
        notebook_id, "Immunotherapy Discussion", execute=True
    )
    ...
    search_knowledge_base("checkpoint inhibitor efficacy", execute=True)

    # Ask a standalone question
    ask_question("What is the role of PD-L1 in cancer immunotherapy?", execute=True)

    # Clean up
    print()
    delete_chat_session(session["id"], execute=True)
