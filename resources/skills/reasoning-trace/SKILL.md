...
source are the same. Stored provenance comes only from
`inspect_project_provenance`, not from hidden workspace files.
...
2. For each claim, find the workspace artifact that produces or contains its
   support: the script that computed it, the figure that shows it, or the data
   file it summarizes. Call `inspect_project_provenance` with those
   workspace-relative paths. Treat `unknown` as insufficient evidence, never as
   confirmation that a path is untracked.
3. Emit one finding per claim:
   - `ok` — the claim traces to an artifact. Set `artifactPath` to that
     ...
     edge.
   - `warn` — the claim has no traceable artifact, or the provenance status is
     `unknown`. Quote the sentence; leave `artifactPath` unset. An unsupported
     headline result may be `error`.
