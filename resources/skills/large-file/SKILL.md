...
zerowall:
  schema_version: 1
  domains: [general]
  research_stages: [observation, analysis]
  roles: [analyst]
...
After `use_skill`, run the absolute `large_file_probe.py` path listed under
`Bundled Resources`:

```text
python "<absolute path to large_file_probe.py>" DATA_FILE [--sample N]
```
