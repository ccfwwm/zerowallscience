"""
Literature-review helpers loaded into ZeroWall's persistent Python kernel when the
skill is used:
...
def litrev_contact() -> str | None:
    ...
    ZeroWall does not expose credentials or profile data inside Python. A user who
    wants to identify these requests may set ``ZEROWALL_LITERATURE_CONTACT_EMAIL``
    in the selected Python environment before starting the kernel.
    """
    import os

    value = os.environ.get("ZEROWALL_LITERATURE_CONTACT_EMAIL", "").strip()
    return value or None


def litrev_openalex_auth() -> str:
    """`&api_key=...` query fragment for api.openalex.org requests, or ""
    when no key is configured. ZeroWall does not inject credentials into Python;
...
def litrev_get(url: str, timeout: float = 15) -> dict | None:
    ...
    c = litrev_contact()
    ua = "ZeroWallScience-literature-review/1.0" + (f" (mailto:{c})" if c else "")
...
def litrev_head(url: str, timeout: float = 10) -> int | None:
    ...
    c = litrev_contact()
    ua = ("ZeroWallScience-literature-review/1.0" + (f" (mailto:{c})" if c else "")).encode(
        "ascii", "ignore"
    ).decode("ascii")
