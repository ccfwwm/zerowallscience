...
class Task:
    ...
    def load(self) -> dict[str, Any]:
        try:
            ...
            return {"schema": 2, "created_at": now(), "stage": "initialized", "single_article": True, "papers": []}
...
def paper_text(paper: Paper) -> str:
    """Prefer the durable MinerU markdown copy over lossy PDF text extraction."""
    if paper.parsed_text_path:
        try:
            return Path(paper.parsed_text_path).read_text(encoding="utf-8")
        except OSError:
            pass
    return extract_pdf_text(Path(paper.pdf_path)) if paper.pdf_path else ""
...
def ingest_mineru_result(task: Task, state: dict[str, Any], paper_identity: str,
    ...
    """Copy a MinerU result into the resumable task and retain its provenance."""
    ...
    destination = task.root / "parsed" / f"{safe_name(paper.pmid or paper.doi or paper.key)}.mineru.md"
    ...
    paper.parsed_text_path = str(destination.resolve())
...
def download_response(client: Client, response: requests.Response, target: Path, paper: Paper, source: str) -> tuple[bool, str, str]:
    ...
    if ok:
        temp.replace(target)
        paper.pdf_path, paper.pdf_source, paper.pdf_sha256, paper.pdf_status = str(target), source, digest, "downloaded"
    ...
    try:
        ...
        if path and path.exists():
            ...
            if ok:
                ...
                paper.pdf_path, paper.pdf_source, paper.pdf_sha256, paper.pdf_status = str(target), url or "authorized_adapter", digest, "downloaded_authorized"
                return True
        ...
        if not ok: paper.pdf_status = f"authorized_adapter_{reason}"
        return ok
    except Exception as exc:
...
def download_via_paper_download(task: Task, paper: Paper, target: Path,
    ...
    try:
        ...
        paper.acquisition_attempts.append({
            "provider": "paper-download", "status": result.status,
            "work_dir": result.work_dir, "at": now(),
        })
...
def download_paper(client: Client, paper: Paper, allow_tsg: bool = False) -> None:
    ...
    for url in urls:
        for attempt in range(1, 4):
            try:
                response = client.session.get(url, timeout=client.timeout, stream=True, allow_redirects=True)
                client.task.record("pdf", response.url, str(response.status_code), paper=paper.key, attempt=attempt)
                paper.acquisition_attempts.append({"provider": "pdf", "url": redact_url(response.url), "status": response.status_code, "attempt": attempt, "at": now()})
                if response.status_code in RETRYABLE_STATUS and attempt < 3:
                    time.sleep(min(2 ** attempt, 8)); continue
                response.raise_for_status()
                ok, reason, _ = download_response(client, response, target, paper, response.url)
                if ok: paper.acquisition_state = "pdf_acquired"; return
    ...
    if os.getenv("LITERATURE_DISABLE_PAPER_DOWNLOAD", "").strip().lower() not in {"1", "true", "yes"}:
        if download_via_paper_download(client.task, paper, target,
            ...
            paper.acquisition_state = "pdf_acquired"
            return
    if download_authorized_adapter(client, paper, target):
        paper.acquisition_state = "pdf_acquired"
        return
    tsg_configured = all(os.getenv(name, "").strip() for name in (
        "TSG_PM_JSESSIONID", "TSG_SESSIONID", "TSG_SGUSER", "TSG_TSGUSER",
    ))
    if allow_tsg and paper.doi and tsg_configured:
        if download_via_tsg(client, paper, target):
            return
    if not paper.pdf_path:
        paper.acquisition_state = "needs_reacquisition"
...
def update_phase_status(state: dict[str, Any], papers: list[Paper]) -> dict[str, Any]:
    ...
    acquisition_attempted = bool(papers) and all(paper.pdf_status != "not_attempted" for paper in papers)
    ...
    state["phase_status"] = {
        ...
        "acquire_pdfs": "complete" if downloaded and len(downloaded) == len(papers) else ("complete_with_gaps" if downloaded else ("complete_no_pdfs" if acquisition_attempted else "pending")),
        "mineru_parse": "complete" if downloaded and len(parsed) == len(downloaded) else ("in_progress" if parsed else ("complete_no_inputs" if acquisition_attempted and not downloaded else "pending")),
        "locate_citation_contexts": "complete" if contexts else ("complete_no_matches" if parsed else ("complete_no_full_text" if acquisition_attempted and not downloaded else "blocked_on_mineru")),
...
def report(task: Task, state: dict[str, Any]) -> None:
    ...
    lines += ["## 三、文章明细", "", "| 方向 | 标题 | DOI | PMID | PDF 状态 | 引用次数 | 国家 | 机构 |", "|---|---|---|---|---|---|---|---|"]
    for paper in papers:
        lines.append(f"| {md(paper.direction)} | {md(paper.title)} | {md(paper.doi)} | {md(paper.pmid)} | {md(paper.pdf_status)} | {md(json.dumps(paper.cited_by_counts, ensure_ascii=False))} | {md(', '.join(paper.countries))} | {md(', '.join(paper.institutions))} |")
    ...
    lines += ["", "## 七、证据边界", "", "元数据、摘要、MinerU 全文、引用上下文和作者证据分层保存。无法获取全文、提供方失败、引用标记含义不确定或人物履历未验证的记录均保留在 Excel 对应工作表中。流程状态不是 complete 时，本报告只是中间产物，不得表述为完整研究已经完成。"]
...
def run_analyze(args: argparse.Namespace) -> int:
    ...
    if args.download_pdfs:
        ...
        with ThreadPoolExecutor(max_workers=workers) as pool:
            futures = {pool.submit(download_paper, client, paper, args.allow_tsg): paper for paper in papers}
