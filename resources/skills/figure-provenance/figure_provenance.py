...
Checks (stdlib only, no network):
  ...
  ok        - filesystem chain resolved and the figure is newest
...
def _mtime(tree: Tree, rel: str) -> float:
    """Filesystem mtime used only as a freshness hint."""
    try:
    ...
    for c in chains:
        ...
        found.append(
            Finding(
                "ok",
                "figure · filesystem",
                f"{c.figure} has a filesystem code/data chain",
                f"Written by {c.generator}"
                + (f"; reads {', '.join(c.inputs[:3])}" if c.inputs else "; no data file read")
                + f"; newest of the chain (mtime {int(_mtime(tree, c.figure))}). "
                "Stored provenance must be checked separately.",
            )
        )
    return found
        ...
        "provenance_paths": sorted(
            {
                path.lstrip("./").replace("\\", "/")
                for chain in chains
                for path in [chain.figure, chain.generator, *chain.inputs]
