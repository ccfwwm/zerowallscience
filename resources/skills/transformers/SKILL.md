...
execution context injects it into the environment without writing a token file:

```bash
python -c "import os; print('HF_TOKEN configured' if os.getenv('HF_TOKEN') else 'missing_runtime: HF_TOKEN')"
```
