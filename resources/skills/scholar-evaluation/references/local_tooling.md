...
PYTHONDONTWRITEBYTECODE=1 python3 scripts/calculate_scores.py \
  ...
  --output .zerowall/tmp/scholar-score.json
```
...
PYTHONDONTWRITEBYTECODE=1 python3 scripts/weight_sensitivity.py \
  --rubric assets/rubric_template.json \
  --evaluation .zerowall/tmp/work-a-evaluation.json \
  --evaluation .zerowall/tmp/work-b-evaluation.json \
  --delta 0.2 \
  --output .zerowall/tmp/weight-sensitivity.json
```
...
PYTHONDONTWRITEBYTECODE=1 python3 scripts/summarize_agreement.py \
  ...
  --output .zerowall/tmp/agreement-summary.json
```
...
PYTHONDONTWRITEBYTECODE=1 python3 scripts/generate_report_scaffold.py \
  --rubric assets/rubric_template.json \
  --evaluation assets/evaluation_template.json \
  --output .zerowall/tmp/developmental-report-scaffold.json
```
...
PYTHONDONTWRITEBYTECODE=1 python3 scripts/generate_report_scaffold.py \
  --rubric assets/rubric_template.json \
  --evaluation .zerowall/tmp/work-a-evaluation.json \
  --traceability .zerowall/tmp/traceability.json \
  --agreement .zerowall/tmp/agreement-summary.json \
  --sensitivity .zerowall/tmp/weight-sensitivity.json \
  --process .zerowall/tmp/process-check.json \
  --output .zerowall/tmp/developmental-report-scaffold.json
```
