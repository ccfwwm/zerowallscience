...
zerowall:
  schema_version: 1
  domains: [bioinformatics, proteomics]
...
When writing code, read the API key only from the `ADAPTYV_API_KEY`
environment variable injected by ZeroWall Settings > Credentials. Never scan
the project for credential files or hardcode tokens.
...
Every request except `GET /openapi.json` requires authentication. Store tokens
in ZeroWall Settings > Credentials and never commit them to source control.
...
**Environment variables** (injected by the approved ZeroWall execution context):
