...
Configure `HF_TOKEN` in ZeroWall Settings > Credentials. The approved
execution context injects it as an environment variable, so scripts can read
`os.environ.get("HF_TOKEN")` without inspecting project files. If it is absent,
fall back gracefully because many resources are public. Never hard-code or echo
the token, and do not traverse the project looking for credentials.
...
- `scripts/fetch_catalog.py` — fetch and filter catalog content. Run with `--help` for full usage. Use this in preference to ad-hoc browser-use or search_mcp_tools calls when you need structured access.
