...
```bash
uv pip install gradio_client    # or: uv add gradio_client
```

For private/gated Spaces, configure `HF_TOKEN` in ZeroWall Settings >
Credentials. The approved execution context injects the variable before the
client starts.
