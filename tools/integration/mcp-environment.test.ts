...
const base = {
  ...
  python: { version: '3.12.10', relativeExecutable: 'Python/python.exe', relativeSitePackages: 'Python/Lib/site-packages', modules: ['mcp', 'numpy', 'pandas', 'httpx'], supportsZeroWallTool: true }, pythonHealth: { imports: ['mcp', 'numpy', 'pandas', 'httpx'], bioServer: 'bio-tools/run_server.py mcp_bio', ketcherServer: 'ketcher-chemistry/server.js' }, skillsRoot: 'skills', sci: { version: '0.3.15', nodeMinimum: '20.3.0', cli: 'sci/dist/cli.mjs', mcp: 'sci/dist/mcp.cjs' }, mcp: { bioToolsVersion: '0.0.37', ketcherChemistryVersion: '0.0.37', sciMasterVersion: '0.3.15', publicToolCount: 8, internalToolCount: 247, servers: ['zerowall_managed_bio_tools', 'zerowall_managed_ketcher', 'zerowall_managed_scimaster'] }, source: { claudeScienceRuntime: '0.0.37-linux-x64', sourceHashes: {} }, signature: { algorithm: 'ed25519', keyId: 'test', value: '' },
} as const
...
describe('MCP environment contract', () => {
  ...
  it('installs only a verified archive and cleans failed temporary output', async () => {
    ...
    try {
      ...
      await expect(installVerifiedArchive(store, manifest, archive, async (_bytes, target) => { await mkdir(join(target, 'Python'), { recursive: true }); await writeFile(join(target, 'Python/python.exe'), 'fake') })).resolves.toContain('slots')
