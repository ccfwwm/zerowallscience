...
export function registerWindowControls(window: BrowserWindow, trustedUrl: () => string | undefined, splashUrl: string, startupContents?: () => WebContents | undefined, quitStartup?: () => void): void {
  ...
  ipcMain.handle(channel, (event, action: unknown) => {
    ...
    if ((event.sender !== window.webContents && event.sender !== startupContents?.()) || frame !== event.sender.mainFrame) throw new Error('Untrusted window')
    ...
    if (action === 'quit-startup' && url.protocol === 'file:' && quitStartup) { quitStartup(); return state() }
