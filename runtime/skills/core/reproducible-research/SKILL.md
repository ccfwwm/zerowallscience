...
```bash
python "$XDG_CONFIG_HOME/opencode/skills/reproducible-research/repro_check.py" [dir]
```
...
- **repro · provenance** — a generated artifact (figure, table, processed data)
  with no record in `.zerowall/provenance.jsonl`.
...
The workspace records every file the agent writes to `.zerowall/provenance.jsonl`
— one JSON object per line, `{path, version, ts, tool, content, …}` with an
epoch-second `ts`. The directory is hidden, so read it with `cat`:

```bash
cat .zerowall/provenance.jsonl
```
