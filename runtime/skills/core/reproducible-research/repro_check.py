...
Checks (stdlib only, no network):
  ...
  provenance  - every generated artifact appears in .zerowall/provenance.jsonl
...
def _provenance_paths(root: str) -> set[str] | None:
    """Paths recorded in .zerowall/provenance.jsonl, or None when there is no log."""
...
def check_provenance(tree: Tree) -> list[Finding]:
    recorded = _provenance_paths(tree.root)
    artifacts = [
        f
        for f in tree.files
    ...
    if recorded is None:
        return [
            Finding(
                ...
                "repro · provenance",
                "No provenance log",
                f"{len(artifacts)} generated artifact(s) present but "
                ".zerowall/provenance.jsonl does not exist - none of them can be "
                "traced back to the run that produced it.",
            )
        ]
    orphans = [f for f in artifacts if f not in recorded]
    if orphans:
        return [
            Finding(
                "warn",
                "repro · provenance",
                f"{len(orphans)} artifact(s) missing from the provenance log",
                ", ".join(orphans[:6]) + ("…" if len(orphans) > 6 else "")
                + " - regenerate them through the workspace, or record where they came from.",
            )
        ]
    return []
...
def run(paths: list[str]) -> dict:
    ...
    findings += check_provenance(tree)
