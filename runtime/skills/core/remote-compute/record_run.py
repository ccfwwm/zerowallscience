#!/usr/bin/env python3
"""Record a remote (SSH/HPC/Modal) experiment run into the ZeroWall Science provenance.
...
record to <workspace>/.zerowall/remote-runs.jsonl, which the app merges into
...
STORE = os.path.join(".zerowall", "remote-runs.jsonl")
ENV_DIR = os.path.join(".zerowall", "env")  # package lockfiles, content-addressed
...
def read_env(path):
    ...
    .zerowall/env/<hash>.txt (mirroring how the app records local runs), and
    ...
    env: dict[str, object] = {
        ...
        "app": os.environ.get("ZEROWALL_APP_VERSION", "unknown"),
    }
    if python:
        env["python"] = python
    if freeze:
...
def main():
    p = argparse.ArgumentParser(description="Record a remote run into ZeroWall Science provenance.")
    ...
    # The skill passes `--session-id "$(cat .zerowall/session.txt)"`, which is
