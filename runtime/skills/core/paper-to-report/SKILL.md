...
```bash
python "$XDG_CONFIG_HOME/opencode/skills/paper-to-report/report_scaffold.py" [dir] --out report.md
```
...
2. **Data** — every data file found, with size and whether the provenance log
   records where it came from. Files with no provenance record are marked, since
   an undocumented input is the most common gap in a shared workspace.
...
4. **Results** — one subsection per figure, each with an embedded image, the
   ...
   uses.
5. **Reproducibility** — the commands to rebuild the figures, the provenance log
   line count, and any figure that has no generator.
