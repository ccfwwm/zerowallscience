...
// `<folder>/.zerowall/project.json`. The folder IS the workspace — sessions
...
fn meta_file(dir: &Path) -> PathBuf {
    dir.join(".zerowall").join("project.json")
}
...
fn populate_import(
    ...
    // provenance/caveats go to an app-owned `.zerowall/IMPORTED_FROM.md`; AGENTS.md
    ...
    // `mark_imported` keeps `.zerowall/` out of git and snapshots go to the
...
/// stale-environment caveats) go to an app-owned `.zerowall/IMPORTED_FROM.md`, so
...
fn record_import_provenance(dir: &Path, source: &Path) {
    ...
    let detail_path = dir.join(".zerowall").join("IMPORTED_FROM.md");
    ...
    let line = format!(
        "\n{IMPORT_PROVENANCE_MARKER}\n> Imported into this workspace by copying from \
         `{}` — see `.zerowall/IMPORTED_FROM.md` for the original location and caveats.\n",
        source.display()
    );
...
///   remove only the `.zerowall/project.json` marker, demoting it to a plain
...
mod tests {
    ...
    fn imported_project_points_at_its_external_source_without_writing_into_it() {
        ...
        assert!(!ext.join(".zerowall").join("project.json").exists());
    ...
    fn provenance_preserves_agents_md_and_is_idempotent() {
        ...
        assert!(agents.contains(".zerowall/IMPORTED_FROM.md"));
        assert_eq!(agents.matches(IMPORT_PROVENANCE_MARKER).count(), 1);
        // Full details live in the app-owned file, with the source path.
        let detail = fs::read_to_string(dir.join(".zerowall").join("IMPORTED_FROM.md")).unwrap();
