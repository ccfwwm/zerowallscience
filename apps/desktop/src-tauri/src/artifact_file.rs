...
mod tests {
    ...
    fn list_dir_sorts_dirs_first_and_skips_hidden() {
        let root = std::env::temp_dir().join(format!("zerowall-listdir-{}", std::process::id()));
    ...
    fn unique_name_dedupes_with_numeric_suffix() {
        let dir = std::env::temp_dir().join(format!("zerowall-unique-test-{}", std::process::id()));
    ...
    fn workspace_relative_detects_in_place_files() {
        ...
        let root = std::env::temp_dir().join(format!("zerowall-wsrel-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        ...
        let outside = std::env::temp_dir().join(format!("zerowall-wsrel-out-{}.png", std::process::id()));
    ...
    fn locate_finds_literal_bare_and_missing_paths() {
        let root = std::env::temp_dir().join(format!("zerowall-locate-test-{}", std::process::id()));
    ...
    fn locate_prefers_the_newest_of_duplicate_basenames() {
        let root = std::env::temp_dir().join(format!("zerowall-locate-dup-test-{}", std::process::id()));
