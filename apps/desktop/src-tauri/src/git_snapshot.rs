...
const AUTHOR_NAME: &str = "ZeroWall Science";
const AUTHOR_EMAIL: &str = "zerowall-science@local";
...
const SNAPSHOT_INDEX: &str = "zerowall-index";
...
fn snapshot_marker(root: &Path) -> PathBuf {
    root.join(".git").join(".zerowall-snapshots")
}

/// Written under a workspace's `.zerowall/` to opt it out of app-managed
...
fn no_snapshot_marker(root: &Path) -> PathBuf {
    root.join(".zerowall").join(NO_SNAPSHOT_MARKER)
}
...
/// keep the app's `.zerowall/` dir out of the user's `git status` via a local
...
pub fn mark_imported(root: &Path) {
    if root.join(".git").is_dir() {
        exclude_locally(root, ".zerowall/");
    } else {
        let osdir = root.join(".zerowall");
...
mod tests {
    ...
    fn importing_a_repo_keeps_it_pristine_and_excludes_the_provenance_dir() {
        ...
        // .zerowall/ so our provenance dir never shows in the user's status.
        assert!(!super::no_snapshot_marker(&root).exists());
        let exclude = fs::read_to_string(root.join(".git/info/exclude")).unwrap();
        assert!(exclude.lines().any(|l| l.trim() == ".zerowall/"));
