import type { Project, Session } from "@zerowall/shared";
import {
  ariCurve,
  biorxivShot,
  citationScatter,
...
plt.style.use("zerowall.mplstyle")  # bundled palette
...
const reproSession: Session = {
  ...
  blocks: [
    ...
    },
    {
      kind: "agent",
      markdown:
        "Reproduced. Final **ARI 0.790** vs. published **0.80** (Δ 0.010, inside the ±0.02 tolerance). The gap traces to the pinned seed. Full environment, hardware, and the exact command are recorded to `.zerowall/runs.jsonl` — one click regenerates it.",
    },
