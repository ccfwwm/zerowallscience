# ZeroWall Science — Prioritized Requirements (Community-Informed)

> **Purpose.** This document turns real community evidence into concrete,
> prioritized requirements for **ZeroWall Science**, our open-source, local-first,
...
ZeroWall Science must win on those four axes — plus two things competitors are
...
Market as: *"ZeroWall Science — local-first, model-agnostic AI research
...
- **Status.** ✅ Every agent write appends a version record to
  `.zerowall/provenance.jsonl` (code, tool, model, session, timestamp, and
  ...
  lockfile `.zerowall/env/<hash>.txt`; the record carries `{count, hash}`, the
...
- **Status.** 🟡 Partial — approval modes shipped 2026-07-05; sidecar auth +
  ...
  **Cleanup — structure (not urgent):**
  - [ ] Split `lib/runtime.ts` (1,014 lines, ~6 concerns): extract the pure
    fold/history reducers and connection/retry; stop hardcoding `ZeroWallScience/`
...
- **Status.** 🟡 One documented chart design system — a validated
  ...
  places (`@zerowall/shared` chartPalette, `index.css --series-*`,
  `zerowall.mplstyle` applied by the `publication-figures` skill) — so agent
