# ZeroWall Science — Technical Design
...
reconsidered. But ZeroWall Science's core is the workbench, files, agent, runtime, and
...
- with an **app-private** config/data dir via `XDG_CONFIG_HOME`/`XDG_DATA_HOME` under
  `~/Library/Application Support/com.zerowall.science/runtime/` (macOS) — so the user's
...
```text
~/.zerowall/
  config/  runtime/{opencode,python,node}/  profiles/zerowall-science/
  workspaces/  logs/  cache/  secrets/
```

Windows: `%APPDATA%/ZeroWall Science/` · macOS: `~/Library/Application Support/ZeroWall Science/`
...
Outputs: `ZeroWall-Science-aarch64.dmg`, `ZeroWall-Science-x64.dmg`,
`ZeroWall-Science-universal.dmg` (later). Code signing / notarization needs an Apple
Developer account; a free account cannot notarize, so users may still see an
"unverified" prompt.

### 12.2 Windows

Outputs: `ZeroWall-Science-Setup.exe`, `ZeroWall-Science.msi` (later). Prefer the NSIS
